...
def multiplier(flag,i,j,n,matrix):          
    direction = []
    for l in range(0,n):
        if flag == 'north':
            y = i - l
        ...
        if (x >= 0) and (x <= 19) and (y >= 0) and (y <= 19):
            direction.append(int(matrix[y][x]))
            #print(list(direction))
        else:
            return 0
    return reduce(lambda x,y: x*y, direction, 1)    



n = 4
total_prod = 0
flags = ['north', 'north_west', 'west', 'south_west', 'south', 'south_east', 'east', 'north_east']
for flag in flags:
    for i in range(0,19):
        for j in range(0,19):
            if total_prod < multiplier(flag,i,j,n,matrix): 
                total_prod = multiplier(flag,i,j,n,matrix)
